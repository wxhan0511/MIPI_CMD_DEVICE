...
**帧格式**：控制命令使用 **64 字节**；升级握手成功后可通过 `0x32`
协商为 **1024 字节**数据帧，两个方向长度一致：
...
1. 主机敲时钟发出当前帧长的命令帧（控制阶段为 64 字节）；
2. Bootloader 处理完毕后，主机用一次"哑读"事务按当前帧长读回应答帧。
...
| 0x29 | WRITE 写入 | [3..6]=App 区内偏移（**必须 4 字节对齐**），[7..8]=数据长度(≤1015)，[9..]=数据 | 状态码 |
| 0x2A | READ 回读校验 | [3..6]=偏移，[7..8]=长度(≤1021) | [3..]=回读数据 |
| 0x30 | JUMP_APP 跳转 | 无 | 状态码（主机读到 OK 后设备即跳转 App） |
| 0x31 | GET_INFO | 无 | 同 SYNC |
| 0x32 | SET_FRAME | [3..4]=帧长度（64 或 1024） | 状态码 |
...
> WRITE 的最后一块允许不足数据区上限，Bootloader 会用 0xFF 补齐到整字再编程；
...
cmd 0x29 -> status 0x00                                ← WRITE 逐帧（协商后每约 1 KB 一条）
