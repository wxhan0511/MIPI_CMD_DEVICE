...
static power_status_func_t power_enable_status[8] = {
    get_VCC_status,
    get_IOVCC_status,
    get_VSP_status,
    get_VSN_status,
    ...
    get_ELVDD_status,
    get_ELVSS_status};
static lim_status_func_t lim_gear_status[8] = {
    get_VCC_lim_status,
...
// 0xff: no secondary d_trigger channel selection needed
...
void task_sample_run(void *argument)
{
    ...
    uint8_t pin_num = 0;
    uint8_t usr_idx = 0;
    // Mapping from the user-provided set-voltage index to the DAC channel index; 5 DACs with 20 channels in total
    ...
    for (;;)
    {
        ...
        case LIM_SET:
            ...
            // Map the user-provided index to the DAC channel index
        ...
        case SINGLE_VOL_GET:
            ...
            // After measuring the 24-pin and 40-pin paths, close their channels to avoid interference
            if (usr_idx == 10)
        ...
        case GET_FREQUENCY:
            ...
            get_freq_flag = 0;
            // Stop outputs that may conflict
            disableTim1PWMOutput();
            disableTim2PWMOutput();

            // Call the adaptive measurement function
            ...
            g_sample_task.cmd_type = NORMAL_LOOP_EVENT;
            break;
        // Gear, timing, and algorithm must stay consistent with the 4.0 sampling board
        case GET_RESISTANCE:
            pin_p = meter_rx_buf[2];
            pin_n = meter_rx_buf[3];
            dev_vol.sample_res_gear_rd = OHM_100_OHM;
            dev_vol.channel_en = 0b00000100; // Enable channel 2 only
            bsp_ads1256_ch2_select(0);       // Select sub-channel 0 of channel 2 first
            ...
            t0 = HAL_GetTick();
            while (latest_sample_ch_sel[2] != 0) // Wait until the sub-channel of ADS1256 channel 2 switches to 0 and the latest sample data is ready
            {
                if ((HAL_GetTick() - t0) >= 5000U)
                {
                    // Can be changed to a timeout status according to the project's status definitions
                    g_sample_task.cmd_status = POWER_CMD_STATUS_TIMEOUT;
            ...
            wait_adc_one_round(100); // Wait at least 6 rounds; gear switching starts from gear 6
            ...
            dev_vol.channel_en = 0b11111111; // Enable all channels
            task_com_resume();
            g_sample_task.cmd_type = NORMAL_LOOP_EVENT;
            break;
        case GET_DIODE:
            ...
            wait_adc_one_round(1000);
            t0 = HAL_GetTick();
            while (latest_sample_ch_sel[2] != 0) // Wait until the ADS1256 channel 2 data is ready
            {
                if ((HAL_GetTick() - t0) >= 1000U) // Wait at most 1 s
                {
                    // Can be changed to a timeout status according to the project's status definitions
                    g_sample_task.cmd_status = POWER_CMD_STATUS_TIMEOUT;
        ...
        case GET_24PIN_VOLTAGE:
            ...
            pin_num = pin_num - 1; // The external pin_num is 1-based; convert to 0-based here
            bsp_select_24pin_channel(pin_num, 1);

            ads1256_ch_index = 2;
            d_trigger_ch_index = 2;
            latest_sample_ch_sel[ads1256_ch_index] = 8; // Set latest_sample_ch_sel to 8 first to indicate that data is not ready
        ...
        case SEL_LIM_GEAR:
            ...
            // RST pulled low when all gears are uA; pulled high if any gear is mA
            uint8_t lim_gear_ua_count = 0;
            for (uint8_t i = 0; i < 8; i++)
                if (i != idx_lim_gear) // Other gears
                    if (!lim_gear_status[i]())
                        lim_gear_ua_count++; // Count the other gears whose current limit is in the uA gear
        ...
        case WRITE_CALI_DATA:
        {
            ...
            // Temporary buffer for assembling the 80 floats
            ...
            // 1. Read the current packet sequence number (0~7)
            uint8_t num = meter_rx_buf[2];
            uint8_t *ptr = &meter_rx_buf[3];

            // Guard against num being out of range (valid values are 0~7)
            if (num > 7)
            {
            ...
            if (num == 7)
            {
                ...
                // TODO: apply the actual calibration or write it to Flash here
                // ================= Start writing calibration values =================

                // 5. Write the DA calibration values
                // Order: vcc_set_gain, vcc_set_offset, iovcc, vsp, vsn, avdd, vdd, elvdd, elvss
                ...
                // 6. Write the AD voltage calibration values
                uint8_t v_idx_map[8] = {0, 1, 7, 3, 5, 6, 4, 2}; // Host data indices corresponding to physical channels 0~7
                for (uint8_t i = 0; i < 8; i++)
                {

                    uint8_t idx = v_idx_map[i];
                    cal->ad_data.ch0_gain[i] = ad_v_cali_data[idx * 2] * 1000; // Even indices are gain
                    cal->ad_data.ch0_offset[i] = ad_v_cali_data[idx * 2 + 1];  // Odd indices are offset
                }

                // 7. Write the AD current calibration values (ADCI gain and offset)
                ...
                // ================= Print the final calibration values after assignment =================
                ...
                M_SPI_DEBUG("\r\n========== 1. Final DA set-voltage calibration values ==========\r\n");
                ...
                M_SPI_DEBUG("\r\n========== 2. Final AD voltage calibration values (physical channels 0~7) ==========\r\n");
                for (uint8_t i = 0; i < 8; i++)
                {
                    M_SPI_DEBUG("PHY_CH%d: gain=%.6f, offset=%.6f\r\n", i, cal->ad_data.ch0_gain[i], cal->ad_data.ch0_offset[i]);
                }
                M_SPI_DEBUG("\r\n========== 3. Final AD mA current calibration values ==========\r\n");
                ...
                M_SPI_DEBUG("%s: gain=%.6f, offset=%.6f\r\n", cur_names[3], cal->ad_data.ch6_gain, cal->ad_data.ch6_offset); // Already negated
                ...
                M_SPI_DEBUG("%s: gain=%.6f, offset=%.6f\r\n", cur_names[7], cal->ad_data.ch1_gain[3], cal->ad_data.ch1_offset[3]); // Already negated
                M_SPI_DEBUG("\r\n========== 4. Final AD uA current calibration values ==========\r\n");
                ...
                M_SPI_DEBUG("%s: gain=%.6f, offset=%.6f\r\n", cur_names[3], cal->ad_data.ch6_gain_ua, cal->ad_data.ch6_offset_ua); // Already negated
                ...
                M_SPI_DEBUG("%s: gain=%.6f, offset=%.6f\r\n", cur_names[7], cal->ad_data.ch1_gain_ua[3], cal->ad_data.ch1_offset_ua[3]); // Already negated
                ...
                M_SPI_DEBUG("%s: gain=%.6f, offset=%.6f\r\n", cur_names[3], cal->ad_data.ch6_gain_ua1, cal->ad_data.ch6_offset_ua1); // Already negated
                ...
                M_SPI_DEBUG("%s: gain=%.6f, offset=%.6f\r\n", cur_names[7], cal->ad_data.ch1_gain_ua1[3], cal->ad_data.ch1_offset_ua1[3]); // Already negated
